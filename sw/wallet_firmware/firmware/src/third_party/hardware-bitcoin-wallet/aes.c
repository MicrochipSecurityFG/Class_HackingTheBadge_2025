...
int aesEncryptECB(const uint8_t *input, size_t input_len,
                  ...
                  size_t *cipher_len)  
{
    ...
    // Calculate total ciphertext length after padding
    padded_len = (input_len % 16 == 0) ? (input_len + 16) : (input_len + (16 - (input_len % 16)));

    // Check if the output buffer is large enough
    if (output_size < padded_len) {
        return -1;  // Not enough space
    }

    // Encrypt full 16-byte blocks
    while (remaining >= 16) {
        aesEncrypt(output + offset, (uint8_t *)(input + offset), expanded_key);
        offset += 16;
        remaining -= 16;
    }

    // Prepare and encrypt final padded block
    memset(block, 0, 16);
    if (remaining > 0) {
        memcpy(block, input + offset, remaining);
    }
    block[remaining] = 0x80;  // ISO9797-1 padding byte

    aesEncrypt(output + offset, block, expanded_key);
    offset += 16;

    // Output the actual ciphertext length
    if (cipher_len != NULL) {
        *cipher_len = offset;
    }
...
int aesDecryptECB(const uint8_t *input, size_t input_len,
                  uint8_t *output, size_t output_size,
                  uint8_t *expanded_key,
                  size_t *plain_len)
{
...
//    }
    ...
    while (offset < input_len) {
        aesDecrypt(output + offset, (uint8_t *)(input + offset), expanded_key);
        offset += 16;
    }

    // Remove ISO9797-1 padding from the final block
    // Scan backwards for the 0x80 byte
    size_t i = input_len;
    while (i > 0 && output[i - 1] == 0x00) {
        i--;
    }

    if (i == 0 || output[i - 1] != 0x80) {
        return -3; // Padding not found or invalid
    }

    // Set final plaintext length
    if (plain_len != NULL) {
        *plain_len = i - 1;
    }
