...
/** Deterministically derive private key from a BIP32 node (a.k.a. extended
  ...
  */
bool bip32DerivePrivate(BigNum256 out, uint8_t *chain_code_out, 
                       const uint8_t *master_node, const uint32_t *path, 
                       const unsigned int path_length)
{
	...
	// Copy final chain code (big-endian, no swap needed unless you want little-endian)
	memcpy(chain_code_out, &current_node[32], 32);

	return false; // success
}

/**
 * Derive the next private key from a BIP-32 node at a specific index.
 * \param out The derived private key will be written here (little-endian 256-bit).
 * \param chain_code_out The derived chain code will be written here (big-endian).
 * \param parent_node The parent node (private key + chain code) to derive from.
 * \param index The child index to derive (0 to 0x7fffffff for non-hardened).
 * \return false on success, true on error.
 */
bool bip32DeriveNextPrivate(BigNum256 out, uint8_t *chain_code_out,
                            const uint8_t *parent_node, uint32_t index)
{
    uint8_t temp[NODE_LENGTH]; // 64 bytes: 32 for I_L, 32 for I_R
    uint8_t hmac_data[37];     // 1 prefix + 33 pubkey/private + 4 index
    uint8_t serialised[ECDSA_MAX_SERIALISE_SIZE];
    uint8_t serialised_size;
    PointAffine p;

    // Check if hardened (not allowed for this function, as it’s meant for non-hardened)
    if (index & 0x80000000) {
        return true; // Hardened derivation not supported here
    }

    // Non-hardened derivation: Use public key of parent
    setToG(&p);
    memcpy(temp, parent_node, 32); // Parent private key
    swapEndian256(temp);           // Big-endian to little-endian for pointMultiply
    pointMultiply(&p, temp);
    serialised_size = ecdsaSerialise(serialised, &p, true);
    if (serialised_size != 33) {
        return true; // Should always be 33 bytes for compressed
    }
    memcpy(hmac_data, serialised, 33);
    writeU32BigEndian(&(hmac_data[33]), index);

    // HMAC-SHA512 with parent chain code as key
    hmacSha512(temp, &(parent_node[32]), 32, hmac_data, sizeof(hmac_data));

    // I_L (first 32 bytes) is the tweak, I_R (last 32) is the new chain code
    swapEndian256(temp);          // Big-endian to little-endian
    swapEndian256((uint8_t *)parent_node);   // Parent private key to little-endian
    if (bigCompare(temp, (BigNum256)secp256k1_n) != BIGCMP_LESS) {
        return true; // I_L >= n
    }

    setFieldToN();
    bigAdd(temp, temp, (uint8_t *)parent_node); // k_i = I_L + k_par (mod n)
    swapEndian256((uint8_t *)parent_node);      // Restore parent to big-endian
    if (bigIsZero(temp)) {
        return true; // k_i == 0
    }

    // Output
    memcpy(out, temp, 32);            // Little-endian private key

    if(chain_code_out != NULL)
    	memcpy(chain_code_out, &temp[32], 32); // Big-endian chain code
