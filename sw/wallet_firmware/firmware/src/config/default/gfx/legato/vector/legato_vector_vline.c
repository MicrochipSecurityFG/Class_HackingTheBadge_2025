                          ...
                          leReal_i16 h)
{
    ...
    y = leReal_i16_ToInt(pnt->y);

    drawPnt.x = leReal_i16_ToInt(pnt->x);
    ih = leReal_i16_ToInt(h);

    if(_context.base.alpha == LE_REAL_I16_ONE)
    {
        for(drawPnt.y = y; drawPnt.y < y + ih; ++drawPnt.y)
        {
...
static void _testPointRd(struct leVector_Kernel* krn)
{
    ...
    if(leReal_i16_AbsoluteValue(krn->testPoint.y) > _context.aabb.extents.y)
        return;

    if(krn->testPoint.y < _context.line.p0.y)
    {
    ...
    else if(krn->testPoint.y > _context.line.p1.y)
    {
    ...
    {
        if(leReal_i16_AbsoluteValue(krn->testPoint.x) <= _context.maxEdgeDistance)
        {
            _kernel.hitCount += 1;
        }
    }
...
static uint8_t _shade_CapNo_Gradient(struct leVector_Kernel* krn)
{
    ...
    distance = leReal_i16_AbsoluteValue(krn->testPoint.x);
...
static uint8_t _shade_CapSq_Gradient(struct leVector_Kernel* krn)
{
    ...
    if(krn->testPoint.y < _context.line.p0.y)
    {
        ...
        {
            distance = leReal_i16_AbsoluteValue(_context.line.p0.y - krn->testPoint.y);
        }
    }
    else if(krn->testPoint.y > _context.line.p1.y)
    {
        ...
        {
            distance = leReal_i16_AbsoluteValue(krn->testPoint.y - _context.line.p1.y);
        }
    }
    else
    {
                        ...
                        const struct leVectorLineAttr* attr)
{
    ...
    _context.minHardnessDistance = LE_REAL_I16_MULTIPLY(attr->hardness, _context.maxEdgeDistance);

    _context.line.p0 = *pnt;
    _context.line.p1.x = pnt->x;
    _context.line.p1.y = _context.line.p0.y + h;

    _context.actualLine = _context.line;

    if(attr->capStyle != LE_CAPSTYLE_NONE)
    {
        _context.actualLine.p0.y -= _context.maxEdgeDistance;
        _context.actualLine.p1.y += _context.maxEdgeDistance;
    }

    _context.aabb.extents.x = _context.maxEdgeDistance;
    _context.aabb.extents.y = leReal_i16_AbsoluteValue(_context.actualLine.p1.y - _context.actualLine.p0.y);
    _context.aabb.extents.y = LE_REAL_I16_MULTIPLY(_context.aabb.extents.y, LE_REAL_I16_ONE_OVER_TWO);
    _context.aabb.origin.x = _context.actualLine.p0.x;
    _context.aabb.origin.y = _context.actualLine.p0.y + _context.aabb.extents.y;
    ...
    _context.actualLine.p0.y = -_context.aabb.extents.y;
    _context.actualLine.p1.x = 0;
    _context.actualLine.p1.y = _context.aabb.extents.y;
