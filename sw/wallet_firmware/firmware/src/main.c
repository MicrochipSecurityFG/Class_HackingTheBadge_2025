...
void UI_CLI_CreateLogin(void) {
	...
	printf("Generating Wallet Login...\n");
...
void UI_EncryptionAttack(void) {
	// Step Encryption Bonus 1
    LCD_MENU_EnterSubMenu();

    // Display initial screen
    LCD_MENU_DisplayTextBuffer("<Encryption Attack>", 0);
    LCD_MENU_DisplayTextBuffer("                   ", 1);

    // Reset menu to known state
    IR_SendMessage("\n", 0, NULL);
    IR_SendMessage("9", 1, NULL);

    sprintf(ui_screen_text, "working...");
    LCD_MENU_DisplayTextBuffer(ui_screen_text, 2);
    LCD_MENU_RefreshScreen();

    // Select "Load Saved Wallet" (option 3)
    if (!IR_SendMessage("5", 1, "Enter PIN:")) {
    ...
    IR_RegisterCallback(UI_HelperEncryptionAttackRXHandler); //Log IR data to FIFO buffer to later print out

    SYSTICK_DelayMs(1000);

    // Send PIN
    IR_SendMessage("1", 0, "Invalid"); // Send dummy PIN get encrypted blob from slot

    sprintf(ui_screen_text, "done");
    LCD_MENU_DisplayTextBuffer(ui_screen_text, 2);
    ...
    while (true) {
		...
		if (writeMenu) {
			if (!wallet.isLoaded) {
				printf("\nNo Wallet Loaded\n");
                //Step Random 1
                printf("1 - Create Login\n");
                printf("2 - Load Wallet\n");

				//Step 2.2 Load Wallet
                printf("3 - Load Saved Wallet\n");  
            ...
            if (!wallet.isLoaded) {
				switch (rx.receiveBuffer[0]) {
					...
					case '4':
						break;
					case '5':
						UI_CLI_RestoreWalletFromPIN_IRMode();
						break;
					case '6':
						UI_HelperEncryptionAttackPrint();
						//reset state
						UI_HelperEncryptionAttackRXHandler(NULL, 0);
						IR_RegisterCallback(NULL);
						break;
					default:
